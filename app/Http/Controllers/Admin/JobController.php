...
class JobController extends Controller
{
    ...
    public function update(Request $request, Job $job)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($job->image) {
                try {
                    ...
                    // Contoh URL: https://res.cloudinary.com/CLOUD_NAME/image/upload/v12345/dira-jobs/jobs/public_id.jpg
                    $pathParts = explode('/', parse_url($job->image, PHP_URL_PATH));
                    // Mengambil bagian 'folder/public_id'
                    $publicIdWithFolder = 'dira-jobs/jobs/' . pathinfo(end($pathParts), PATHINFO_FILENAME);
        ...
        $job->update([
            ...
            'location' => $validatedData['location'],
            'deadline' => $validatedData['deadline'],
            'image' => $imageUrl, // Update dengan URL baru atau yang lama
        ]);
